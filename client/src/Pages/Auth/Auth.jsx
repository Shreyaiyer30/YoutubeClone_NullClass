...
//       // Instead of dispatching login immediately, call API to check OTP requirement
//       const { data } = await api.login({ email: user.email, name: user.displayName || name, region });

//       if (data.otpRequired) {
//         setTempUserId(data.result._id);
//         setShowOtpInput(true);
//         alert(data.message);
//       } else {
//         // No OTP required (fallback or old logic), dispatch standard login
//         dispatch(login({ email: user.email, name: user.displayName || name, region }));
//         // Actually if API returned the token, we should just use it. 
//         // But `dispatch(login)` re-calls API. To avoid double call, better to dispatch {type: AUTH, data} directly?
//         // The current action calls API.
//         // Since I already called API and got data, I should just dispatch directly.
...
//         alert("Invalid Email or Password. Please duplicate check your credentials or Sign Up if you don't have an account.");
